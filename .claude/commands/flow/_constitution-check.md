...
TIER=$(grep -o "TIER: \(Light\|Medium\|Heavy\)" .flowspec/memory/constitution.md | cut -d' ' -f2)
TIER=${TIER:-Medium}  # Default to Medium if not found

# Count NEEDS_VALIDATION markers
MARKER_COUNT=$(grep -c "NEEDS_VALIDATION" .flowspec/memory/constitution.md || echo 0)

# Extract section names from NEEDS_VALIDATION markers
SECTIONS=$(grep "NEEDS_VALIDATION" .flowspec/memory/constitution.md | sed 's/.*NEEDS_VALIDATION: /  - /')
