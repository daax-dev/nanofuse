...
The `/jpspec:reset` command:
1. Re-prompts for validation mode on each workflow transition
2. Regenerates `jpspec_workflow.yml` with new settings
...
if [ ! -f "jpspec_workflow.yml" ] && [ ! -d ".claude" ]; then
...
If no `jpspec_workflow.yml` exists but `.claude/` directory does, this is a valid project that just needs workflow configuration.
...
Write this file to `./jpspec_workflow.yml`.
...
2. If constitution already exists, warn user:
   ```
   Warning: Existing constitution at .specify/memory/constitution.md will be replaced.
   This will NOT preserve any custom modifications.

   Continue? [y/N]: _
   ```

3. Copy the selected tier template to `.specify/memory/constitution.md`
...
📁 FILES UPDATED
  ...
  ✓ jpspec_workflow.yml (regenerated)
  {✓ .specify/memory/constitution.md (if --constitution)}

🎯 NEXT STEPS

1. Review the generated jpspec_workflow.yml
2. Commit the configuration:
   git add jpspec_workflow.yml
   git commit -s -m "chore: reconfigure workflow validation modes"

💡 TIP: Run '/jpspec:assess <feature>' to start a new workflow
```
...
This command mirrors the behavior of the CLI's internal `prompt_validation_modes()` and `generate_jpspec_workflow_yml()` functions. Users can also reconfigure via CLI:
...
Before completing:
- [ ] jpspec_workflow.yml exists and is valid YAML
