...
hooks:
  pre-commit:
    - name: "security-scan-staged"
      ...
      script: ".specify/scripts/security/pre-commit-scan.sh"
      on_failure: "warn"  # or "block" to prevent commit
EOF

# Create pre-commit script
cat > .specify/scripts/security/pre-commit-scan.sh << 'EOF'
#!/bin/bash
set -e
...
chmod +x .specify/scripts/security/pre-commit-scan.sh
```
...
# Test manually
bash .specify/scripts/security/pre-commit-scan.sh
```
