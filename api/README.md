...
A request that only sends `X-SPIFFE-ID` without a verified client certificate is rejected; if a verified certificate is present, the certificate URI SAN is the identity source and headers are ignored.
