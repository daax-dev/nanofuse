...
|------|------------|-------------------|-----------------------------|-----------------|---------------------|
| Nanofuse | Self-hosted `nanofused` REST API over Unix socket or TCP | Firecracker microVM on Linux/KVM | Not yet. Current API manages VM/image/snapshot/log lifecycle. | Create/start/stop/kill/delete VMs; pull images; snapshots are partially implemented. | Self-hosted microVM control plane with host-owned networking, egress policy, image materialization, and local storage. |
