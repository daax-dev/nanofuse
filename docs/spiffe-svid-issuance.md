...
- **Atomic, restrictive writes.** Each issuance writes a temp file
  ...
  swap and the credential is never world/group-readable.
...
- Full TOCTOU-safe writes against a hostile parent directory. The Manager does
  best-effort path permission checks and a post-write symlink/mode guard, and
  assumes the runtime places the mount under a root-owned, non-attacker-writable
  parent (e.g. `/var/run/secrets/spiffe`). Eliminating the residual race
  requires fd-relative (`openat`/`O_NOFOLLOW`) writes, which are platform
  specific and belong to the runtime guest-mount integration.
